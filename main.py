...
while True:
    try:
        # 识别下一个数字
        leftPicNext = pyautogui.screenshot(region=(175, 425, 60, 40))   # 左侧的下一个数字，需修改
        rightPicNext = pyautogui.screenshot(region=(320, 425, 60, 40))  # 右侧的下一个数字，需修改
        ...
        # 识别题号
        jud = pyautogui.screenshot(region=(225, 63, 100, 32))  # 题目区域，需修改
        jud.save('jud.png')
        ...
        if number == 1:
            leftPic = pyautogui.screenshot(region=(125, 310, 85, 58))  # 左侧的当前数字，需修改
            rightPic = pyautogui.screenshot(region=(315, 310, 85, 58))  # 右侧的当前数字，需修改
        ...
        if number != pre:   # 保证每题只画一次
            ...
            if dict[number][0] > dict[number][1]:
                pyautogui.moveTo(229, 675, duration=0)      # 画笔区域，需修改
